...
// Must have error, proprotional term (KP) derivative (KD) integral (KI) scaled value
...
void lineRunner(){
    ...
    float derivative = 0; // constant derivative of errors.
    int derSig = 0; // used to scale motors.

    float integral = 0; // constant integral of errors.
    ...
    while(true){ // will have to eventually break this to turn it off once the quadrant has been completed and the sensors detect walls.
        ...
        derSig = (difference/ePeriod) * derivative; // constant

        previousVal = eVal;
        error += eVal; //Sums error.

        pSig = eVal*kp;
        intSig = (error * integral);
